...
class Data<T> {
	...
	public void add(Object... values) {
		for (Object value : values) {
			if (data instanceof Double && value instanceof Number) {
				data = (T) Double.valueOf((Double) data + ((Number) value).doubleValue());
			} else if (data instanceof Integer && value instanceof Number) {
				data = (T) Integer.valueOf((Integer) data + ((Number) value).intValue());
			} else if (data instanceof String) {
				data = (T) (data.toString() + value.toString());
			} else {
...
class Table {
	...
	public double parseFunc(String in) {
		...
		switch (func) {
		case "SUM":
			return sumColDouble(colName);
		case "AVG":
			return avgCol(colName);
		default:
...
public class Database {
	...
	public static void main(String[] args) {
		...
		String r = "2,4.0,Test;3,5.0,Test2";
		d.addTable(name, c, r);
		Table t = d.getTable(name);
		TESTING.print(t.toStringTable());
		TESTING.print(t.parseFunc("avg(Hello)"));
	}
}
