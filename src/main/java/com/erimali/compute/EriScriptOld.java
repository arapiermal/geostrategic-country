...
public class EriScriptOld {
	...
	private Map<String, EriScriptOld> functions;
	private int i;
	...
	public void initThings() {
		...
		this.forLoops = new Stack<Integer>();
		this.variables = new HashMap<>();
		this.functions = new HashMap<>();
		this.rows = new ArrayList<String>();
		this.printed = new LinkedList<String>();
		this.i = 0;
	...
	public EriScriptOld(String string) {
		this.forLoops = new Stack<Integer>();
		this.variables = new HashMap<>();
		this.varArr = new HashMap<>();
		this.varString = new HashMap<>();
	...
	public void uniteRows(StringBuilder sb, String[] splitted, char separator) {
		...
		if (separator != '~') {
			...
			if (endSign != -1)
				sb.append(splitted[i].substring(0, endSign));
		} else {
			int endSign = -1;
			while (i < splitted.length && (endSign = splitted[i].indexOf('~')) == -1) {
	...
	public void loadFunction(String[] splitted) {
		...
		if (hasInputs == -1) {
			...
			while (++i < splitted.length && !(splitted[i] = splitted[i].trim()).startsWith("#@")) {
				fRows.add(splitted[i]);
			}
			functions.put(fName, new EriScriptOld(fRows, null));
		} else {
			...
			functions.put(fName, new EriScriptOld(fRows, params));
		}
	}
...
//PARAM FROM LAST TIME REMEMBERED !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	...
	public EriScriptOld(List<String> fRows, String[] params) {
		initThings();
		this.rows = fRows;
		this.params = params;
	}
...
//double vs int can become problematic
	...
	public void execFuncRow() {
		...
		// caching??
		String[] parts = rows.get(i).split("(?<!\\\\):");// will always be same?!?
	...
	private void skip() {
		while (++i < rows.size() && !rows.get(i).startsWith(".")) {
		}
	}
	...
	private void execIf() {
		...
		int a = rows.get(i).indexOf(':');
		if (parseBool(rows.get(i).substring(a + 1))) {
			isTrue = true;
			execute(++i);
		} else {
			skip();
		...
		////////////////////:::::::::::::::::::::
		//WOOOOOOOOOOOOOOW
		while (i < rows.size() && (b = rows.get(i).lastIndexOf(':')) > 0) {
			if (isTrue) {
				skip();
			} else {
				if (parseBool(rows.get(i).substring(b + 1))) {
					isTrue = true;
					execute(++i);
				} else {
					skip();
		...
		if (rows.get(i).toUpperCase().contains("ELSE")) {
			if (isTrue) {
				skip();
			} else {
				execute(++i);
	...
	public EriScriptOld executeFunction(String name, double... values) {
		if (functions.containsKey(name)) {
			EriScriptOld f = functions.get(name);
			f.execute(values);
	...
	public String replaceFuncsWithVals(String input) {
		...
		while ((t = input.indexOf('@')) != -1) {
			...
			if (functions.containsKey(fName)) {
				EriScriptOld f = executeFunction(fName, inputsVal);
